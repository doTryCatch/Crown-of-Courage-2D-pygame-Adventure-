...
SCREEN_WIDTH = 800
SCREEN_HEIGHT = 600
root_path = "C:/Users/Roshan/Desktop/python/crown of courage/"
            ...
            try:
                ...
                if 0 < index <= 90:
                    ...
                    try:
                        ...
                        for k in range(
                            find_file_length(f"/Assets/Level1/1-1/Extras/Tile_{index}")
                        ):
                            img.append(
                                loadimages(
...
def print_text(text, pos, speaker):
    ...
    if speaker == "player1":
        text_rect.topleft = (
            player1.x - tail_offset - text_rect.width,
            player1.y - text_rect.height - box_offset,
        )
        tail_pos = (text_rect.right, text_rect.bottom)
        tail_end = (tail_pos[0] + tail_offset, tail_pos[1] + tail_offset)
    else:
        text_rect.topright = (
            player2.x + tail_offset + text_rect.width,
            player2.y - text_rect.height - box_offset,
        )
